...
class Gadget {
	constructor(opts){
		...
		console.log("deviceClass from json File: "+ opts.bDeviceClass)
		this.bDeviceClass= opts.bDeviceClass || '0x00',
		this.bDeviceSubClass= opts.bDeviceSubClass || '0x00',
		this.bDeviceProtocol= opts.bDeviceProtocol || '0x00',
	...
	loadConfigs(configs){
		return configs.map(c=>{
			...
			c.files.forEach(file => {
				if(!file.name || file.name =="") console.log("invalid file configuration");
			})
			return c
		})
	}
	...
	create(cb){
		...
		batch.writeFile('bDeviceProtocol', this.bDeviceProtocol)
		batch.writeFile('bDeviceSubClass', this.bDeviceSubClass)
		batch.writeFile('bDeviceClass', this.bDeviceClass)
		...
		this.functions.forEach(f=>{
			...
			if(f.type == 'hid')
			{				
				batch.writeFile('protocol', f.protocol)
				batch.writeFile('subclass', f.subclass)
				batch.writeFile('report_desc',f.report)
				batch.writeFile('report_length',f.report.length)
			}
			f.files.forEach(file=>{
				
				batch.mkdir(file.path);
				batch.setBase(file.path);
				batch.writeFile(file.name, file.content);
				batch.setBase(fbase);
			});
